...
export default async function handler(req, res) {
  // #region agent log
  console.log('[DEBUG:1f530b] users/export/ids INVOKED', JSON.stringify({method:req.method,hasApiKey:!!process.env.BRAZE_API_KEY,hasEndpoint:!!process.env.BRAZE_REST_ENDPOINT,ts:Date.now()}));
  // #endregion
  ...
  try {
    // #region agent log
    console.log('[DEBUG:1f530b] users/export/ids FORWARDING to Braze', JSON.stringify({targetUrl,ts:Date.now()}));
    // #endregion
    ...
    // #region agent log
    console.log('[DEBUG:1f530b] users/export/ids BRAZE RESPONDED', JSON.stringify({status:brazeResponse.status,ts:Date.now()}));
    // #endregion
