/**
 * Vercel Serverless Function — POST /api/braze/users/track
 *
 * Proxies the Braze /users/track endpoint server-side so the
...
export default async function handler(req, res) {
  ...
  if (!BRAZE_API_KEY || !BRAZE_REST_ENDPOINT) {
    console.error('[braze-proxy] Missing env vars for users/track');
  ...
  try {
    ...
    console.error('[braze-proxy] Network error on users/track:', err.message);
